...
class InitializerCommand extends Commander {
  ...
  ArgParser get argParser => ArgParser()
    ..addOption("android-package-name", abbr: 'a', help: 'Package name for the application.', defaultsTo: BuildInfo.androidPackageName)
    ..addOption("ios-package-name", abbr: 'i', help: 'Bundle identifier for the iOS application.', defaultsTo: BuildInfo.iosBundleId)
  ...
  Future? run() async {
    ...
    _logWelcome();
    _checkPubspecFile();
    await _createDirectory(path.join("distribution", "android", "output"), initialized, "android_directory");
    if (Platform.isMacOS) {
      await _createDirectory(path.join("distribution", "ios", "output"), initialized, "ios_directory");
    }

    if (!(argResults!['skip-tools'] as bool)) {
      await _checkTool("flutter", "Flutter", initialized, args: ['--version']).catchError((_) => -1);
      await _checkTool("firebase", "Firebase", initialized).catchError((_) => -1);
      await _checkTool("git", "Git", initialized, args: ['--version']).catchError((_) => -1);

      await CompressFiles.checkTools().then((value) {
        if (!value) {
          logger.logWarning("Compress tool is not installed, some features may not work as expected.");
        } else {
      ...
      await _checkTool("fastlane", "Fastlane", initialized, args: ['actions']).catchError((_) => -1).then((value) async {
        if (value == 0) {
          ...
          if (Platform.isWindows) {
            logger.logWarning('Tips : In Windows, you can install fastlane using "gem install fastlane", make sure you have Ruby installed.');
          } else if (Platform.isLinux) {
            logger.logWarning('Tips : In Linux, you can install fastlane using "sudo gem install fastlane", make sure you have Ruby installed.');
          } else if (Platform.isMacOS) {
            logger.logWarning('Tips : In MacOS, you can install fastlane using "sudo gem install fastlane", make sure you have Ruby installed.');
          }
          initialized["fastlane_json"] = false;
        }
      });
      if (Platform.isMacOS) {
        await _checkTool("xcrun", "XCRun", initialized, args: ["altool", "-h"]).catchError((_) => -1);
      }
    }
  ...
  Future<int> _checkTool(String command, String toolName, Map<String, bool> initialized, {List<String> args = const []}) async {
    logger.logDebug("Checking if $toolName is installed...");
    logger.logDebug("Command: $command ${args.join(" ")}");
    final process = await Process.start(command, args, includeParentEnvironment: true, runInShell: true).then((value) async {
      ...
      if (await value.exitCode != 0) {
        ...
        logger.logWarning('$toolName is not installed. Some features may not work as expected.');
  ...
  Future<int> _validateFastlaneJson(Map<String, bool> initialized) async {
    ...
    File jsonKeyFile = jsonKeyPath != null ? File(jsonKeyPath) : Files.fastlaneJson;

    if (!jsonKeyFile.existsSync()) {
      logger.logWarning("The Fastlane JSON key file does not exist at the specified path: ${jsonKeyFile.path}");
      initialized["fastlane_json"] = false;
      return -1;
    }
    return await Process.start("fastlane", ['run', 'validate_play_store_json_key', 'json_key:${jsonKeyPath ?? Files.fastlaneJson.path}'], runInShell: true, includeParentEnvironment: true)
        .then((value) async {
      value.stdout.transform(utf8.decoder).listen(logger.logDebug);
      if (await value.exitCode != 0) {
        initialized["fastlane_json"] = false;
  ...
  Future<void> _downloadAndroidMetaData() async {
    ...
    if (argResults?['android-package-name'] == null || (argResults!['android-package-name'] as String).isEmpty) {
      logger.logError("Android package name is required to download metadata. Please provide it using --android-package-name option.");
      return;
    }
          ...
          Task(
            ...
            jobs: [
              Job(
                ...
                builder: BuilderJob(android: android_arguments.Arguments(Variables.fromSystem(globalResults), binaryType: "aab", buildMode: "release")),
              ),
              Job(
